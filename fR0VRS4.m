...
%y=1;
%No vacunados
dx1 = Lambda - (b4*x(1)*(x(3)+x(9))./N)*(p./x(11)) - (phiv4+d)*x(1) + psi*x(4) + phi*x(7); %S
dx2 = (b4*x(1)*(x(3)+x(9))./N)*(p./x(11)) - (delta+d)*x(2); %E
dx3 = delta*x(2) - (sigma4 + g + d)*x(3); %I
dx4 = g*x(3) + gH*x(5) - (psi + d)*x(4); %R
%Hospitalizados
dx5 = sigma4*x(3) - (gH + dH)*x(5) - w*y*x(5)+ gH*x(6); %H 
dx6 = w*y*x(5) - (gH+dH)*x(6); %Hc
%Vacunados
dx7 = phiv4*x(1) - (bv4*x(7)*(x(3)+x(9))./N)*(p./x(11)) - (phi+d)*x(7) + psiv*x(10); %Sv
dx8 = (bv4*x(7)*(x(3)+x(9))./N)*(p./x(11)) - (deltav + d)*x(8); %Ev 
